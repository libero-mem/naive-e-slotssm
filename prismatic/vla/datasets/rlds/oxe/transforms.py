...
########################################################################
########################################################################
# def quaternion_to_euler_tf(quaternions):
#     # Split the quaternion tensor into w, x, y, z components
#     w, x, y, z = tf.split(quaternions, 4, axis=1)
    
#     # Compute Roll (x-axis rotation)
#     roll = tf.atan2(2 * (w * x + y * z), 1 - 2 * (x**2 + y**2))
    
#     # Compute Pitch (y-axis rotation)
#     pitch = tf.asin(tf.clip_by_value(2 * (w * y - z * x), -1.0, 1.0))  # Clip to avoid invalid values due to numerical issues
    
#     # Compute Yaw (z-axis rotation)
#     yaw = tf.atan2(2 * (w * z + x * y), 1 - 2 * (y**2 + z**2))
    
#     # Convert radians to degrees if needed
#     roll = tf.math.degrees(roll)
#     pitch = tf.math.degrees(pitch)
#     yaw = tf.math.degrees(yaw)
    
#     # Concatenate the results into a single tensor of shape [N, 3]
#     euler_angles = tf.concat([roll, pitch, yaw], axis=1)
    
#     return euler_angles

# def gripper_qpos_to_single_variable(gripper_qpos, qpos_min=0.22, qpos_max=0.30):
#     # Split the input tensor into qpos1 and qpos2 components
#     qpos1, _ = tf.split(gripper_qpos, 2, axis=1)
    
#     # Combine the two gripper joint positions (average)
#     gripper_position = qpos1
    
#     # Normalize to the range [-1, 1]
#     normalized_gripper_position = (2 * (gripper_position - qpos_min) / (qpos_max - qpos_min)) - 1
    
#     return normalized_gripper_position
...
def robomimic_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trajectory["observation"]["proprio"] = tf.concat(
        [
            ...
            trajectory["observation"]["robot0_eef_quat"],
            trajectory["observation"]["robot0_gripper_qpos"]
        ],
        axis=1,
    )
...
def real_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # trajectory["observation"]["EEF_state"] = trajectory["observation"]["cartesian_position"][:, :6]
    # trajectory["observation"]["gripper_state"] = trajectory["observation"]["gripper_position"][:, -1:]
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "robomimic_lift": robomimic_dataset_transform,
    "robomimic_can": robomimic_dataset_transform,
    "robomimic_square": robomimic_dataset_transform,
    "robomimic_tool_hang": robomimic_dataset_transform,

    ### LIBERO Finetuning datasets
    "libero_spatial": libero_dataset_transform,
    "libero_object": libero_dataset_transform,
    "libero_goal": libero_dataset_transform,
    "libero_goal_oc": libero_dataset_transform,
    "libero_90": libero_dataset_transform,
    "libero_10": libero_dataset_transform,
    "libero_relation": libero_dataset_transform,
    "libero_mem": libero_dataset_transform,

    ### Real dataset
    "real_data": real_dataset_transform,
    "real_action": real_dataset_transform,
}
