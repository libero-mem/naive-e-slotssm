...
def augment(obs: Dict, seed: tf.Tensor, augment_kwargs: Union[Dict, Dict[str, Dict]]) -> Dict:
    ...
    image_names = {key[6:] for key in obs if key.startswith("image_") and 'seg' not in key}
...
def decode_and_resize(
    ...
    for name in image_names:
        ...
        if name in resize_size:
            if '_seg' in name:
                image = tf.image.resize(image, resize_size[name], method="nearest")
            else:
    ...
    for name in depth_names:
        ...
        if depth.dtype == tf.string:
            if tf.strings.length(depth) == 0:
                ...
                depth = tf.io.decode_image(depth, expand_animations=False, dtype=tf.uint8)
