...
# modified compared to originally in terms of data type
def resize_depth_image(depth_image: tf.Tensor, size: Tuple[int, int]) -> tf.Tensor:
    """Resizes a depth image using bilinear interpolation. Expects & returns uint8 in arbitrary range."""
    assert depth_image.dtype == tf.uint8
    ...
    depth_image = tf.cast(tf.clip_by_value(tf.round(depth_image), 0, 255), tf.uint8)
    return depth_image
